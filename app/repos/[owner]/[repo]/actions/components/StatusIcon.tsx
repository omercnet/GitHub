...
function mapStatus(status: string, conclusion: string | null): { color: string; label: string } {
  if (status === 'completed') {
    switch (conclusion) {
      ...
      case 'failure': return { color: 'bg-red-500', label: 'failure' }
      case 'cancelled': return { color: 'bg-gray-500', label: 'cancelled' }
      case 'skipped': return { color: 'bg-gray-500', label: 'skipped' }
      default: return { color: 'bg-gray-500', label: conclusion || 'completed' }
    }
  }
